...
def team_properties():
    properties = dict()
    player_names = ["Cursed Kane", "Панчевалдо", "Елмасалдо"]
    properties['team_name'] = "No league :("
    properties['player_names'] = player_names
    properties['image_name'] = 'BM.png' # use image resolution 153x153
...
def decision(our_team, their_team, ball, your_side, half, time_left, our_score, their_score):
    manager_decision = [dict(alpha=3.14/2), dict(), dict()]
    for i in range(3):
        ...
        manager_decision[i]['alpha'] = player[dict()] # player['alpha'] # choose direction for running (0, 2*pi)
